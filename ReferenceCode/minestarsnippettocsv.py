...
parser = OptionParser()
parser.add_option("-S", "-- SourceFolder", action="store", dest="sourcefolder", type="string", help="Snippet Source Path")
...
def ProcessSnippetFile(Filename):
    #Log("Processing :"+ os.path.basename(Filename))
    if os.path.getsize(Filename) < 36:
        #print "File too small. Skipping ... "
        return False
    BaseFilename = os.path.basename(Filename)
    with open(Filename, mode='rb') as snippetfile:
        fileContent = snippetfile.read()
        if ValidateSnippetfile(fileContent):
            BufferLen = len(fileContent)
            Offset = 0
            Index = 20
            while (arcpy.Index < BufferLen):
                if struct.unpack_from('b', fileContent, Index)[0] == 11:
                    Offset = Index + 1
                    # was 36
                    if (Offset + 40 > BufferLen):
                        break

                    Item = struct.unpack_from('LLLLLLLLLL', fileContent, Offset)
                    ParsedItems = ParseItem(Item, BaseFilename)
                    for ParsedItem in ParsedItems:
                        if ParsedItem:
                            ItemKey = "{0}_{1}".format(str(ParsedItem['X']), str(ParsedItem['Y']))
                            # If the X,Y exists and we have a later timestamp, replace the item with the later one
                            if ItemKey in Points:
                                if Points[ItemKey]['Timestamp'] <= ParsedItem['Timestamp']:
                                    ParsedItem['Z']= numpy.average([ParsedItem['Z'], Points[ItemKey]['Z']])

                                    Points[ItemKey] = ParsedItem
                            else:
                                Points[ItemKey] = ParsedItem
                    index = Offset + 39
                index += 1
        else:
            #print "File did not validate as a Minestar snippet file. Skipping ... "
            return False
...
try:
    ...
    if os.path.isfile(options.outputcsvfile):
        ...
        print("Getting snippet file list ... ")        
        SnippetFiles = get_file_list(options.sourcefolder, options.filefilter)
        ...
        for filename in sorted(SnippetFiles):
            if os.path.isfile(filename):
                try:
                    ProcessSnippetFile(filename)
                except:
        ...
        print("Total Files: "+ str(len(SnippetFiles)))
